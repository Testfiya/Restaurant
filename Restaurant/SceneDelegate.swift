...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, restoreInteractionStateWith stateRestorationActivity: NSUserActivity) {
        ...
        case .menu(let category):
            let menuTableViewContoller = storyboard.instantiateViewController(identifier: restorationController.identifier.rawValue, creator: { (coder) in return MenuTableViewController(coder: coder, category: category)
            })
            categoryTableViewController.navigationController?.pushViewController(menuTableViewContoller, animated: true)
        case .menuItemDetail(let menuItem):
            ...
            let menuItemDetailViewController = storyboard.instantiateViewController(identifier: restorationController.identifier.rawValue) { (coder) in return MenuItemDetailViewController(coder: coder, menuItem: menuItem)}
